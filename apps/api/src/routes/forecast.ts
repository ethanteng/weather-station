...
import { OpenMeteoClient, Forecast7DayResponse } from '../clients/openMeteo';

const router = Router();
const client = new OpenMeteoClient();

// In-memory cache: Map<cacheKey, { data: Forecast7DayResponse, expiresAt: number }>
const cache = new Map<string, { data: Forecast7DayResponse; expiresAt: number }>();
 ...
 */
function getCachedForecast(cacheKey: string): Forecast7DayResponse | null {
 ...
 */
function setCachedForecast(cacheKey: string, data: Forecast7DayResponse): void {
  cache.set(cacheKey, {
    data,
